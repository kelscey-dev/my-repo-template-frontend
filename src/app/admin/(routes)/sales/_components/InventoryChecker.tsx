...
type InventoryCheckerTypes =
  | {
      ...
      disabled: boolean;
      loading: boolean;
      setExcludeInventoryOptions: React.Dispatch<React.SetStateAction<any[]>>;
      selectionOption: {
        count: number;
        results: any[];
      };
      ...
      selectionData: any;
      fieldKey: number;
      salesStatus: SalesStatusTypes | undefined;
      selectedData: SalesModalData;
    }
  | {
      type: "blur";
      list: {
      ...
      loading?: never;
      salesStatus?: never;
      setExcludeInventoryOptions?: never;
...
export default function InventoryChecker(props: InventoryCheckerTypes) {
  ...
  const inventoryID = useWatch(
    ["sales_details", fieldKey, "inventory_id"],
    form
  );

  const quantity = useWatch(["sales_details", fieldKey, "quantity"], form);
  ...
  const {
    ...
    inventoryID,
    {
      queryKey: ["inventory-transactions", inventoryID, selectedData?.sales_id],
      axiosConfig: {
        params: {
          take: 10,
          excluded_sales_id: selectedData?.sales_id,
  ...
  React.useEffect(() => {
    if (isSuccess) {
      form?.setFieldValue(
        ["sales_details", fieldKey, "breakdown"],
        filterDataByQuantity(inventoryData, quantity).slice(0, -1)
      );

      form?.setFieldValue(
        ["sales_details", fieldKey, "total"],
      ...
      >
        ...
        >
          <Select.Infinite
            ...
            onChange={async (id, prevId) => {
              ...
              if (!id) {
                return setExcludeInventoryOptions!((prevState) =>
        ...
        >
          <NumericFormat
            ...
            prefix="₱ "
            disabled={salesStatus === "completed"}
          />
        </Form.Item>
